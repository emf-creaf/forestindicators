...
The first step is to decide which indicators are to be estimated. You can examine the names of the available indicators in the package by calling function `available_indicators()`:

```{r}
available_indicators()
```

Say we decide to estimate the *timber harvest volume* (`"timber_harvest"`) and the *density of dead wood* (`"density_dead_wood"`). We may need to check which data inputs are required, their units, etc. We can find this information using function `show_information()`:
...
```{r}
show_information("density_dead_wood")
```


### Step 2: Assemble inputs and define values for additional parameters

Once the format of inputs is know, it is the responsibility of the user to build the necessary data frames to be used as inputs. In our case, we will use `example_plant_dynamic_input` that already contains the necessary data for the estimation of the two indicators.

However, we will normally need to define additional parameters to fine-tune the estimation of indicators, For that, we define a **named list** where each element corresponds to a different indicator and, in turn, contains a named list of parameters. In our case:
